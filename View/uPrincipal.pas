...
uses
  ...
  TLHelp32,PsAPI,Registry, JvXPCore, JvXPButtons;
...
uses WinInet, uProdutos, uPessoa, uGrupo, uUnidade, uContas, UpLANO, uCaixa,
  ...
  uAtualizadorAutomatico, uClassificacao_Master;
...
begin
  Dados.aMenu := 'InventrioporCSOSNCST1';
  try
...
begin
  Dados.aTag := 2;
  Dados.aMenu := 'AjustaEstoqueFiscal1';
  try
...
begin
  Dados.aMenu := 'AjustaSaldodeEstoqueFiscal1';
  try
...
begin
  Dados.aMenu := 'MNDevoluodeMercadoria';
  if FTDI = nil then
  begin
    Align := alClient;
...
//    end;
 ...
 // if FileExists(Dir) then
  //  ShellExecute(0, 'open', PChar(Dir), '', '', SW_MINIMIZE);
end;
...
begin
  ...
  StatusBar1.Panels[4].Text := 'Versão:' + Dados.GetBuildInfo
    (Application.ExeName);
