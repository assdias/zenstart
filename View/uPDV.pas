...
uses
  ...
  frxDBSet, frxExportBaseDialog, frxExportPDF, ACBrTEFD, Vcl.ToolWin,
  System.ImageList, Vcl.ImgList, JvExStdCtrls, JvEdit, JvExComCtrls, JvComCtrls;
...
type
  TFrmPDV = class(TForm)
    ...
    pnlFoodsMesas: TPanel;
    DBCtrlGrid1: TDBCtrlGrid;
    ItemMesa: TPanel;
    imgLivre: TImage;
    ImgOcupado: TImage;
    DBText2: TDBText;
    pnlFoodsStatusMesa: TPanel;
    Label11: TLabel;
    DBText3: TDBText;
    DBText4: TDBText;
    PanelFoods: TPanel;
    ...
    dbtxtPDVTotal: TDBText;
    PopupMenu1: TPopupMenu;
    actTEF: TAction;
    actDesconto: TAction;
    lblGeralRestaurante: TLabel;
    ...
    PanelDelivery: TPanel;
    DBGridD: TDBGridEh;
    pnlDeliveryContato: TPanel;
    pnlDeliveryCliente: TPanel;
    edtCliente: TLabel;
    pnEndereco: TPanel;
    edtFone: TMaskEdit;
    DBGridBuscaD: TDBGridEh;
    pnlDeliveryItem: TPanel;
    pnlDeliveryItemProduto: TPanel;
    edtProdutoD: TEdit;
    pnlItemObservacao: TPanel;
    edtObsD: TEdit;
    DBMemo1: TDBMemo;
    ...
    pnlGrid: TPanel;
    pnlPDVLateral: TPanel;
    dbimgProduto: TDBImage;
    pnlGridContainer: TPanel;
    qryProdutoFoto: TFDQuery;
    dsProdutoFoto: TDataSource;
    qryProdutoFotoFOTO: TBlobField;
    pnlFoodsLateral: TPanel;
    dbimgRestauranteProdutoFoto: TDBImage;
    pnlDeliveryContainer: TPanel;
    pnlDeliveryLateral: TPanel;
    dbimgDeliveryProdutoFoto: TDBImage;
    pnlFoodsGrids: TPanel;
    fpnlComandos: TFlowPanel;
    btnCaixa: TLabel;
    ...
    nbTotais: TNotebook;
    lblGeralD: TLabel;
    pnlPDVItemQuantidade: TPanel;
    lblPDVItemQuantidade: TLabel;
    edtQtdP: TEdit;
    pnlPDVItemValorUnitario: TPanel;
    lblPreco: TLabel;
    edtPrecoP: TEdit;
    pnlPDVItemTotal: TPanel;
    lblTotalP: TLabel;
    lblPDVItemTotal: TLabel;
    pnlPDVContainer: TPanel;
    shpDivPDVQuantidade: TShape;
    shpDivPDVPreco: TShape;
    shpDivPDVTotal: TShape;
    lblPDVItemProduto: TLabel;
    shpPDVItemProduto: TShape;
    lblFoodsProduto: TLabel;
    shpFoodsProdutoDiv1: TShape;
    lblFoodsItemObs: TLabel;
    Shape2: TShape;
    pnlFoodsIten: TPanel;
    pnlFoodsItemQuantidade: TPanel;
    lblFoodsItemQuantidade: TLabel;
    shpFoodsItemQuantidadeDiv: TShape;
    pnlFoodsTotalItem: TPanel;
    lblTotalR: TLabel;
    Label3: TLabel;
    Shape4: TShape;
    pnlFoodsItemPreco: TPanel;
    lblFoodsItemPreco: TLabel;
    shpFoodsItemPreco: TShape;
    lblDeliveryFone: TLabel;
    shpDeliveryFoneDic: TShape;
    pnlDeliveryClienteFone: TPanel;
    lblDeliveryCliente: TLabel;
    shpDeliveryClienteDiv: TShape;
    ilImagens: TImageList;
    pnlDeliveryPage: TPanel;
    pnlDeliveryPageButtons: TPanel;
    imgDeliveryPageItens: TImage;
    imgDeliveryPageAnotacao: TImage;
    nbDeliveryPage: TNotebook;
    lblDeliveryAnotacao: TLabel;
    shpDeliveryAnotacaoDiv: TShape;
    pnlDeliveryItem2: TPanel;
    pnlDeliveryItemTotal: TPanel;
    lblTotalD: TLabel;
    lblDeliveryItemTotalLib: TLabel;
    shpDeliveryItemTotalDiv: TShape;
    pnlDeliveryItemQuantidade: TPanel;
    lblDeliveryItemQuantidade: TLabel;
    shpDeliveryItemQuantidadeDev: TShape;
    pnlDeliveryItemPreco: TPanel;
    lblDeliveryItemPreco: TLabel;
    shpDeliveryItemPrecoDiv: TShape;
    EdtPrecoD: TEdit;
    EdtQtdD: TEdit;
    shpDeliveryProdutoDiv: TShape;
    lblDeliveryProduto: TLabel;
    shpDeliveryItemObservacao: TShape;
    lblDeliveryItemObservacao: TLabel;
    ...
    procedure qryPesqProdAfterScroll(DataSet: TDataSet);
    procedure qryItemAfterScroll(DataSet: TDataSet);
    procedure nbPagePageChanged(Sender: TObject);
    procedure ShowPDV(Sender: TObject);
    procedure ShowFoods(Sender: TObject);
    procedure ShowDelivery(Sender: TObject);
    procedure FuncoesLeave(Sender: TObject);
    procedure FuncoesEnter(Sender: TObject);
    procedure ShowDeliveryPageItens(Sender: TObject);
    procedure ShowDeliveryPageAnotacao(Sender: TObject);
  private
...
//  if Screen.Width = 1024 then
  if pnlGridContainer.Width = 1024 then
  begin
  ...
  //pnlPDVItemQuantidade.Width := round(pnlPDVItem2.Width * 0.21);
  //pnlPDVItemValorUnitario.Width := round(pnlPDVItem2.Width * 0.21);
  //pnlPDVItemValorUnitario.Width := round(pnlPDVItem2.Width * 0.22);
      ...
      case Dados.qryConfigMODELO_BALANCA.Value of
        3:
          begin
            ...
            if nbPage.PageIndex = 0 then // PDV
            begin
            ...
            if nbPage.PageIndex = 1 then // Restaurante
            begin
            ...
            if nbPage.PageIndex = 2 then // Delivery
            begin
        ...
        1, 2, 4:
          begin
            ...
            //TODO: TROCAR PARA O CAMPO "FRACIONADA"
...
begin
  try
    ...
    begin
      ...
      if PanelFoods.Enabled and edtProdutoR.Enabled  then
...
begin
  ...
  if qryItemTTOTAL.Value > 0 then
    lblGeralRestaurante.Caption := FormatFloat('0.00', qryItemTTOTAL.Value)
  else
    lblGeralRestaurante.Caption := FormatFloat('0.00', 0);
...
begin
  if (Key in [#13]) and edtProdutoD.Enabled then
...
begin
  if nbPage.PageIndex = 0 then
  begin
    if (Key = #13) and edtQtdP.Enabled then
      edtQtdP.SetFocus;
  end;
  if nbPage.PageIndex = 1 then
  begin
    if Key = #13 then
    begin
      if PanelFoods.Enabled and edtOBSR.Enabled then
        edtOBSR.SetFocus;
    end;
  end;
  if nbPage.PageIndex = 2 then
  begin
    if (Key = #13) and edtObsD.Enabled then
      edtObsD.SetFocus;
  end;
...
begin
  if (ehCaixaRapido = 'N') and EdtQtdD.Enabled then
    EdtQtdD.SetFocus;
end;

procedure TFrmPDV.edtObsDKeyPress(Sender: TObject; var Key: Char);
begin
  if (Key = #13) and EdtQtdD.Enabled then
    EdtQtdD.SetFocus;
end;

procedure TFrmPDV.edtOBSRExit(Sender: TObject);
begin
  if PanelFoods.Enabled then
  begin
    if (ehCaixaRapido = 'N') and edtQtdR.Enabled then
      edtQtdR.SetFocus;
  end;
end;
...
begin
  if (Key = #13) and edtQtdR.Enabled then
    edtQtdR.SetFocus;
end;
...
begin
  ...
  if nbPage.PageIndex = 2 then // Restaurante
  begin
    if Pos('*', Descricao) = 0 then
      edtQtdR.Text := '1';
...
begin
  ...
  if nbPage.PageIndex <> 1 then // se não for restaurante
  begin
...
procedure TFrmPDV.nbPagePageChanged(Sender: TObject);
begin
  if nbPage.PageIndex = 0 then
  begin
    Dados.FTIpoPDV := 'PDV';
  ...
  nbTotais.PageIndex := nbPage.PageIndex;
...
begin
  ...
  begin
    DBGridR.Refresh;
    DBGridR.Repaint;
    if PanelFoods.Enabled and DBGridR.Enabled then
      DBGridR.SetFocus;
  end;
  if nbPage.PageIndex = 2 then
  begin
...
begin
  ...
  if nbPage.PageIndex = 0 then
  begin
    if PanelPDV.Enabled and EdtProdutoP.Enabled then
      EdtProdutoP.SetFocus;
  end;
  ...
  begin
    if Op = 'I' then
    begin
      if PanelPDV.Enabled and edtProdutoD.Enabled then
        edtProdutoD.SetFocus;
    end
    else
    begin
      if PanelDelivery.Enabled and edtFone.Enabled then
        edtFone.SetFocus;
    end;
  end;
...
begin
  ...
  begin
    ...
    try
      ...
      frmFechavenda.PageControl1.ActivePageIndex := 1;
...
begin
  if Dados.qryMesasSITUACAO.Value = 'O' then
    PanelFoods.Enabled := true
  else
    PanelFoods.Enabled := false;
end;
...
begin
  ...
  begin
    ...
    pnlFoodsMesas.Enabled := false;
    PanelDelivery.Enabled := false;
    raise Exception.CreateFmt('Caixa aberto em %s !', [
      FormatDateBr(Dados.qryConsulta.FieldByName('DATA_ABERTURA').AsDateTime)
    ]);
  end
  else
  begin
...
procedure TFrmPDV.ShowDelivery(Sender: TObject);
begin
  nbPage.PageIndex := 2;
end;

procedure TFrmPDV.ShowDeliveryPageItens(Sender: TObject);
begin
  nbDeliveryPage.PageIndex := 0;
end;

procedure TFrmPDV.ShowFoods(Sender: TObject);
begin
  nbPage.PageIndex := 1;
end;

procedure TFrmPDV.ShowPDV(Sender: TObject);
begin
  nbPage.PageIndex := 0;
end;
...
begin
  ...
  qryProdutoFoto.Params[0].AsInteger := DataSet.FieldByName('ID_PRODUTO').AsInteger;
  qryProdutoFoto.Open;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        result := false;
        raise Exception.CreateFmt('Estoque Negativo (%f)!', [qryPesqProdSERVICO.AsExtended]);
      end;
    end
    else
    begin
...
procedure TFrmPDV.qryPesqProdAfterScroll(DataSet: TDataSet);
begin
  qryProdutoFoto.Close;
  qryProdutoFoto.Params[0].AsInteger := DataSet.FieldByName('CODIGO').AsInteger;
  qryProdutoFoto.Open;
end;
    ...
    try
      ...
      begin
        ...
        begin
          ...
          begin
            if nbPage.PageIndex = 0 then
            begin
              if (ActiveControl = EdtProdutoP) and EdtProdutoP.Enabled then
                EdtProdutoP.SetFocus;
            end;
            if nbPage.PageIndex = 1 then
            begin
            ...
            if nbPage.PageIndex = 2 then
            begin
              if (ActiveControl = edtProdutoD) and edtProdutoD.Enabled then
                edtProdutoD.SetFocus;
            end;
      ...
      begin
        ...
        begin
          ChamaGrade;
          ChamaSerial;
          if edtQtdP.Enabled then          
            edtQtdP.SetFocus;
        end;
        if nbPage.PageIndex = 1 then
        begin
          if edtOBSR.Enabled then
            edtOBSR.SetFocus;
        end;
        if nbPage.PageIndex = 2 then
        begin
          if edtObsD.Enabled then
            edtObsD.SetFocus;
        end;
...
var
   sInput: string;
begin
  //todo: tela para selação das unidades
  sInput := InputBox('Qtde', 'Qtde', sInput);

  if (sInput <> '') then
  begin
    TEdit(Sender).Text := sInput;
  end;
...
begin
  ...
  begin
    ...
    if nbPage.PageIndex = 1 then
    begin
      if PanelFoods.Enabled and edtPrecoR.Enabled then
        edtPrecoR.SetFocus;
    end;
    if nbPage.PageIndex = 2 then
      EdtPrecoD.SetFocus;
...
//  nbPage.Pages[0].TabVisible := false;
//  nbPage.Pages[1].TabVisible := false;
//  nbPage.Pages[2].TabVisible := false;

  btnPDV.Visible := false;
  btnFoods.Visible := false;
  ...
  begin
    nbPage.PageIndex := 0; // PDV
    btnPDV.Visible := true;
  end;

  if Dados.FTIpoPDV = 'DELIVERY' then
  begin
    // nbPage.Pages[2].TabVisible := true;
    btnDelivery.Visible := True;

    if Dados.qryTerminalPDV.Value = 'S' then
      btnPDV.Visible := true;
      //nbPage.Pages[0].TabVisible := true;

    if Dados.qryTerminalRESTAURANTE.Value = 'S' then
      btnFoods.Visible := true;
//      nbPage.Pages[1].TabVisible := true;

    nbPage.PageIndex := 2; // Delivery
    nbDeliveryPage.PageIndex := 0; // Delivery Itens
    edtFone.Text := '';
  end;
...
//      nbPage.Pages[1].TabVisible := true;
    ...
    nbPage.PageIndex := 1; // Restaurante;
...
procedure TFrmPDV.FuncoesEnter(Sender: TObject);
begin
  if Sender is TGraphicControl then
  begin
    case TGraphicControl(Sender).Tag of
      1 : lblPDV.Font.Color := $0051A8FF;
      2 : lblFoods.Font.Color := $0051A8FF;
      3 : lblDelivery.Font.Color := $0051A8FF;
    end;
  end;
end;

procedure TFrmPDV.FuncoesLeave(Sender: TObject);
begin
  if Sender is TGraphicControl then
  begin
    case TGraphicControl(Sender).Tag of
      1 : lblPDV.Font.Color := $000684F4;
      2 : lblFoods.Font.Color := $000684F4;
      3 : lblDelivery.Font.Color := $000684F4;
    end;
  end;
end;
...
begin
  ...
  pnlFoodsMesas.Enabled := campos;
  PanelDelivery.Enabled := campos;
...
procedure TFrmPDV.ShowDeliveryPageAnotacao(Sender: TObject);
begin
  nbDeliveryPage.PageIndex := 1;
end;
